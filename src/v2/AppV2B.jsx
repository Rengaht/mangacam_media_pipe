...
function AppV2B({removeBg}) {
  ...
  function onImageSegment(result) {
    ...
    // console.log("mask: ", maskWidth, maskHeight, width, height);
    ...
    //   // if(maskVal>0) console.log("maskVal: ", maskVal);
    //   imageData[j] = 0.0;
  ...
  function drawSword(landmarks){
    ...
    if(hand.visibility > 0.5){
      ...
      const sword=refSwords.current[refIndexBg.current%2];
  ...
  useEffect(()=>{
    ...
    switch(state){
      case STATE.INTRO:
        ...
        // setIndexBg((indexBg+1)%BG_COUNT);
        refIndexBg.current=(refIndexBg.current+1)%BG_COUNT;
  ...
  function toggleOutro(open, callback){

    console.log("toggleOutro: ", open);

    if(open){
      
      gsap.fromTo('#_outro>.outro_mask', {height:'50%'},{
        height:'0%',
        duration: SCENE_FADE_TIME,
        ease: "power4.inOut",
        onComplete: callback,
      });
      
    }else{
      gsap.fromTo('#_outro>.outro_mask', {height:'0%'},{
        height:'50%',
        duration: SCENE_FADE_TIME,
        ease: "power4.inOut",
        onComplete: callback,
  ...
  //   console.log("indexBg changed: ", indexBg);
  ...
  useEffect(()=>{
    ...
    sword1.src="/image/sword.png";
    
    const sword2=new Image();
    sword2.src="/image/sword-3.png";
  ...
  return (
    <div className={`h-full aspect-[250/225] relative overflow-hidden`}>
      ...
      {/* debug area */}
      {/* <section className='absolute top-0 left-0 z-20 flex items-center justify-center'>
        <button className='text-white bg-red-500' onClick={()=>setIndexBg((indexBg+1)%BG_COUNT)}>update bg</button>
        <button className='text-white bg-red-500' onClick={()=>toggleOutro(true)}>open</button>
        <button className='text-white bg-red-500' onClick={()=>toggleOutro(false)}>close</button>
      </section> */}
